...
public interface ProductRepository extends JpaRepository<Product,Long> {
    ...
    @Query(value ="Select * from Product where category_id = :id and id != :productId order by id desc limit 8",nativeQuery = true)
    List<Product> findByCategoryId(long id,long productId);

    @Query(value ="Select * from Product where category_id = :id order by id desc",nativeQuery = true)
